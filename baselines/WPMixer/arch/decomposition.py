...
class Decomposition(nn.Module):
    def __init__(self,
        ...
        self.dwt = DWT1DForward(wave = self.wavelet_name, J = self.level).cuda() if self.device.type == 'cuda' else DWT1DForward(wave = self.wavelet_name, J = self.level)
        self.idwt = DWT1DInverse(wave = self.wavelet_name).cuda() if self.device.type == 'cuda' else DWT1DInverse(wave = self.wavelet_name)
